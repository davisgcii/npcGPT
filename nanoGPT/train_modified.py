...
def get_batch(split):
    ...
    ix = torch.randint(no_ex, (batch_size,))
...
def plot_results(iterations, values, path, ylabel, title):
    plt.clf()
    plt.xlabel("Iteration")
    plt.ylabel(ylabel)
    plt.title(title)
    plt.plot(np.linspace(0, iterations - 1, iterations).astype(int), values)
    plt.savefig(path)
...
def estimate_loss():
    ...
    for k in range(batches):
        X, img, Y = get_fixed_batch('val', k * batch_size)
        with ctx:
            logits, loss, acc = model(X, img, Y)
        losses[k] = loss.item()
    out['val'] = losses.mean()
...
# helps estimate an arbitrarily accurate accuracy over either split using many batches
@torch.no_grad()
def estimate_acc():
    out = {}
    model.eval()
...
# training loop
X, img, Y = get_batch('train') # fetch the very first batch
t0 = time.time()
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        plot_results(1 + iter_num, train_losses, os.path.join(results_dir, "train_losses.png"), "Loss", "Training loss over time")
        plot_results(1 + iter_num, train_accuracies, os.path.join(results_dir, "train_accuracies.png"), "Accuracy", "Training accuracy over time")
        ...
        val_accuracies.append(accs['val'].item())
        print(f"iter {iter_num}: validation loss {losses['val'].item()}, validation acc {accs['val']*100}%, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
        
        plot_results(1 + iter_num, val_losses, os.path.join(results_dir, "val_losses.png"), "Loss", "Validation loss over time")
        plot_results(1 + iter_num, val_accuracies, os.path.join(results_dir, "val_accuracies.png"), "Accuracy", "Validation accuracy over time")
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                print(f"\tsaving checkpoint to {out_dir} for validation loss {best_val_loss} and validation accuracy {best_val_acc}")
                
                # keep only the top 3 checkpoints
                files = sorted(os.listdir(out_dir))[-3:]
                for file_name in os.listdir(out_dir):
                    if file_name not in files:
                        os.remove(os.path.join(out_dir, file_name))
                
                torch.save(checkpoint, os.path.join(out_dir, f'ckpt_{format(iter_num, "04d")}_{torch.round(best_val_loss, decimals=3)}.pt'))
    iter_num += 1
    local_iter_num += 1
